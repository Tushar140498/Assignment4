...
def test_app_menu_command(capfd, monkeypatch):
    inputs = iter(['menu', 'exit'])  # Simulate input for REPL
    monkeypatch.setattr('builtins.input', lambda _: next(inputs))
    ...
    # Check that the output contains "Available commands:" and at least one command
    assert "Available commands:" in out, "MenuCommand should output available commands"
    assert "Exiting the program..." in out, "Application should exit properly"